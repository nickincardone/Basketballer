...
class ScheduleBuilder(object):
    ...
    def build_schedule(self):
        ...
        while current_date < self.today:
            self.get_games(self.date(current_date))
...
class Game(object):
    ...
    def __init__(self, date, game_data):
        self.home = game_data['personalInfo'][
            'home_team']['team-code']['global-id']
        self.away = game_data['personalInfo'][
            'away_team']['team-code']['global-id']
        self.date = date
        self.game_data = game_data
        self.game_data['pbp'] = game_data['pbp'][
            'sports-scores']['nba-scores']['nba-playbyplay']['play']
        self.num_quarters = int(
            game_data['gameInfo']['total-quarters']['total'])
        self.shots = []
    ...
    def get_quarter_starters(self, quarter, play_index):
        ...
        if quarter == 0:
            ...
            for starter in range(10):
                if int(pbp[starter]['global-player-id-1']) in self.home_players:
                    quarterStarters[0].append(
                        int(pbp[starter]['global-player-id-1']))
                else:
                    quarterStarters[1].append(
                        int(pbp[starter]['global-player-id-1']))
        ...
        for i, play in enumerate(pbp):
            if play['event-id'] == '14':
                self.home_quarter_starters.append(home_starters)
                self.away_quarter_starters.append(away_starters)
            ...
            if play['event-id'] == '10':
                ...
                subbed_players.append(player)
                if (old_player not in home_starters and old_player not in away_starters):
                    if (old_player not in subbed_players):
                        if (int(play['team-code-1']) == int(self.home)):
                            home_starters.append(old_player)
                        else:
                            away_starters.append(old_player)
                    continue

            if (player not in home_starters and player not in away_starters):
                if (player not in subbed_players):
                    if (int(play['team-code-1']) == int(self.home)):
        ...
        for play in pbp:
            ...
            if play_type == 10:
                ...
                if old_player in cur_home:
                    cur_home = [
                        player if player != old_player else new_player for player in cur_home]
                elif old_player in cur_away:
                    cur_away = [
                        player if player != old_player else new_player for player in cur_away]
                else:
                    print 'error'
        return
    ...
    def add_shot(self, play, cur_home, cur_away):
        ...
        if (int(play['team-code-1']) == int(self.home)):
            # on home team
            for player in cur_home:
                # checks to see if teammate is himself
                if(player != int(play['global-player-id-1'])):
                    shot['teammates'].append(player)
            for player in cur_away:
                shot['opponents'].append(player)
        else:
            # on away team
            for player in cur_away:
                # checks to see if teammate is himself
...
if __name__ == '__main__':
    ...
    game_data = Basketballer(schedule)
    game_data.get_games()
